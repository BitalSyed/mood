...
// Sample goals data but just for testing purposes
        ...
        <CardDescription>
          {goal.start_date && (
            <span className="flex items-center text-xs">
              ...
              {goal.end_date &&
                ` - ${format(parseISO(goal.end_date), "MMM dd")}`}
...
const GoalManager = ({ averageMood }: { averageMood: number }) => {
        ...
        await fetch(url + "/api/auth/authenticate", {
          ...
          .then((data) => {
            if (data.error != undefined && data.error != "") {
              ...
              // data: {_id: '67f02e433c85f911cd975c99', username: 'a', email: 'a@a.com', data: '', __v: 0} ==> data(.then(data=>{})).data(data: {_id...}).goals('required goals field')
              storedGoals = data.data.goals;
            }
          });
      }
      let goalsData = storedGoals ? JSON.parse(storedGoals) : [];

      // Return empty array if no goals were found
      if (!storedGoals) {
        return [];
      }
  ...
  const createGoal = async () => {
    ...
    try {
      ...
      if (storedUser) {
        await fetch(url + "/api/user/goal", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            jwtToken: storedUser,
            goal: JSON.stringify(updatedGoals),
          }),
        })
          .then((response) => response.json())
          .then((data) => {
  ...
  const completeGoal = async (id: string) => {
    try {
      ...
      const updatedGoals = goals.map((goal) =>
        goal.id === id
          ? { ...goal, completed: true, updated_at: new Date().toISOString() }
          : goal
      );
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground">
          No goals set yet. Create your first goal to start tracking your
          progress.
        </p>
      )}
    </div>
